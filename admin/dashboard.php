  ...
  <bo <div class="flex">
    ...
    <div id="assignModal"
      ...
      <div class="bg-white border border-gray-600 rounded-lg px-6 py-6 shadow-lg relative h-fit w-full">
        <h2 class="flex flex-col gap-4">Assign Phone to Team Leader</h2>
        ...
        <form id="assignForm">
          ...
          <div class="flex justify-end gap-2">
            <button type="button" id="closeAssignModal" onclick="closeAssignModal()"
              class="w-24 px-4 py-2 border border-black text-black rounded-lg hover:bg-gray-200">
              Cancel
            </button>

            <button type="submit"
              class="w-24 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-700">Assign</button>
          </div>
        </form>
      </div>
    </div>
    ...
    assignForm.addEventListener("submit", function (event) {
      ...
      fetch("assign_phone.php", {
        ...
        body: JSON.stringify({
          ...
          hfId: teamLeaderId, // Pass hfId instead of TL ID
        }),
      })
  ...
  function openAssignModal(serial, model) {
    ...
    fetch("fetch_team_leaders.php")
      ...
      .then((data) => {
        ...
        data.forEach((user) => {
          let displayName =
            user.formattedName || `(${user.hfId}) ${user.username}`;
